...
int getVotersFromFile(char* inputFile) {
    ...
    BF* bloomFilter = initializeBloomFilter(numberOfVoters);
    RBT* redBlackTree = initializeRedBlackTree(&alphanumericCompare);
    
    readVotersAndUpdateStructures(input, bloomFilter, redBlackTree);
    printRBT(redBlackTree->root, 0);
    printf("Max level %d\n", maxLevel);
    return 0;
}